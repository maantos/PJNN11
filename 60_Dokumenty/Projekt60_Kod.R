...
work_dir <- "C:/Users/antos/OneDrive/Pulpit/60_Dokumenty"
setwd(work_dir)
...
topics_dir <- "./projektOutput/topics"
plots_dir <- "./projektOutput/plots"
clouds_dir <- "./projektOutput/clouds"
clusters_dir <- "./projektOutput/clusters"
...
dtm_tfidf_1_20 <- DocumentTermMatrix(
  corpus,
  control = list(
    weighting = weightTfIdf,
    bounds = list(
...
"tdm_tf_all.csv"
 ...
 ######Reduction
# analiza głównych składowych (Nalezalo odpowiednio pozmieniac parametry aby przeprowadzic eksperymt dla wszystkich 3 macierzy
...
own_terms <- c("lewandowski","strzelec","piłkarz","kopernika","hobbit",
               "biskup","astronom","kapituła","bramka","mecz","kopernik","piłsudski")
